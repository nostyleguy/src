...
void AlterSchedulerNamespace::validateObject(Object const *object)
{
	...
	if (!isInvalid)
	{
		...
		{ //lint !e1775 // catch block does not declare any exception // that's right: I want to catch it all.
			WARNING(true, ("validateObject(): Object-derived class with address [%x] failed to return valid C++ type info; likely a deleted Object.", reinterpret_cast<uintptr_t>(object)));
			isInvalid = true;
		}
	}
#endif
